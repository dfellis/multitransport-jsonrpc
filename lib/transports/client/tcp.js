...
function onData(data) {
    ...
    while(nullSpot !== -1) {
        var messageSize = this.buffer.substring(0, nullSpot);
        // TODO is messageSize is not a num, then advance
        var totalMessageLength = 2 + messageSize.length + Number(messageSize);
        // Return if We don't have a full message yet
        if (this.buffer.length < totalMessageLength) {
            return;
        }
        var message = this.buffer.substr(messageSize.length + 1, messageSize);
...
TcpTransport.prototype.request = function request(body, callback) {
    ...
    if(this.con) this.con.write(message.length + '\0' + message + '\0');
};
